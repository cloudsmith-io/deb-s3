...
class Deb::S3::Package
  ...
  def url_filename(codename)
    @url_filename || "pool/#{codename}/#{self.component}/#{self.name[0]}/#{self.name[0..1]}/#{File.basename(self.filename)}"
  end

  def url_filename_encoded(codename)
    @url_filename || "pool/#{codename}/#{self.component}/#{self.name[0]}/#{self.name[0..1]}/#{s3_escape(File.basename(self.filename))}"
  end
